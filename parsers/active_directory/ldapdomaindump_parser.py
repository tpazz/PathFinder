...
class _CaseInsensitiveRow:
    """Wraps a csv.DictReader row so key lookups are case-insensitive."""

    def __init__(self, row):
        self._data = {k.lower(): v for k, v in row.items() if k is not None}
...
def parse_ldapdomaindump_dir(dir_path):
    ...
    Uses case-insensitive column lookups so that both 'samaccountname'
    and 'SamAccountName' (etc.) are handled across ldapdomaindump versions.
